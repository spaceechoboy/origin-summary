...
function donut(chains) {
  // 도넛·헤드라인은 현재가치(holding) 기준. 소각채권은 소각된 LGNS라 holding=0 → 도넛에서 빠지고
  // DAI 기준 별도 라인으로만 표시된다(이미 파괴된 수량을 '보유 LGNS'로 세지 않는다).
  let tot = chains.reduce((a, c) => a + c.holding_lgns, 0) || 1, off = 0, segs = "", C = 439.8;
  chains.forEach(c => { let len = c.holding_lgns / tot * C; segs += '<circle cx="90" cy="90" r="70" fill="none" stroke="' + c.color + '" stroke-width="22" stroke-dasharray="' + len.toFixed(1) + ' ' + C + '" stroke-dashoffset="' + (-off).toFixed(1) + '" transform="rotate(-90 90 90)"/>'; off += len; });
  return '<svg viewBox="0 0 180 180" width="168" height="168"><circle cx="90" cy="90" r="70" fill="none" stroke="#1c2230" stroke-width="22"/>' + segs +
    '<text x="90" y="84" text-anchor="middle" fill="#c9d1d9" font-size="16" font-weight="800">' + f0(tot) + '</text><text x="90" y="103" text-anchor="middle" fill="#8b949e" font-size="9">LGNS · 현재가치</text></svg>';
}
function renderSummary() {
  let s = DATA.summary, n = s.notional, chs = Object.keys(s.chains).map(k => { let c = s.chains[k]; return { key: k, name: c.name, holding_lgns: c.holding_lgns, color: COLOR[k] || "#888" }; }).filter(c => c.holding_lgns > 0);
  let tot = n.holding_lgns || 0;
  let lgd = chs.map(c => '<div class="li"><span class="sw" style="background:' + c.color + '"></span><span class="nm">' + esc(c.name) + '</span><b>' + f2(c.holding_lgns) + '</b><span class="pc">' + (tot ? (c.holding_lgns / tot * 100).toFixed(1) : 0) + '%</span></div>').join("");
  let html = '<div class="sum"><div>' + donut(chs) + '</div><div style="flex:1 1 280px"><div class="big">' + f2(tot) + '<span class="u">LGNS 현재가치 (명목 합산)</span></div>' +
    ...
    '<div class="red">예치 원금 <b>' + f2(n.principal_lgns) + ' LGNS</b> <span style="font-size:11px;color:#8b949e">(리베이스·해제분·extra 제외 · 소각채권 소각분 포함)</span></div>' +
...
function chainBlock(c, walletTotal, dappStaked) {
  let rows = c.products.map(p => {
    ...
    return '<tr><td>' + esc(p.label) + ' <span class="cnt">' + p.count + '</span>' + cd + '</td><td>' + f2(p.principal_lgns) + '</td><td class="pur">' + f2(p.holding_lgns) + '</td><td class="dim">' + pct(p.holding_lgns, walletTotal) + '</td><td class="grn">' + f2(p.unlocked_lgns) + '</td><td>' + reb + '</td><td>' + ext + '</td><td>' + usd(p.usd_total) + '</td><td>' + usd(p.usd_total_after_tax) + '</td></tr>';
  }).join("");
  ...
  const bbLine = ((c.burn_bond || {}).groups || []).map(g =>
    ...
    '<span>총 지급예정</span><b>' + f2(g.total_owed_dai) + ' DAI</b>' +
    '<span>수령</span><b class="dim">' + f2(g.paid_dai) + ' DAI</b>' +
    '<span>남은 미지급</span><b class="grn">' + f2(g.remaining_owed_dai) + ' DAI</b>' +
