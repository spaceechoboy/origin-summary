...
// dApp Invite "Anubis Staked"는 정의가 다른 참조 지표 — 합계에 섞이면 안 되고 대조용으로만 실린다.
test('dapp_anubis_staked: 지갑 객체에 실리되 보유 합계엔 영향 없음', () => {
  const positions = [longPoly(W1), commAnu(W1)];
  const withRef = buildWallets([{ wallet: W1, label: 'w', positions, dappAnubisStaked: 9672.1541 }], DISPLAY, PRICES);
  const without = buildWallets([{ wallet: W1, label: 'w', positions }], DISPLAY, PRICES);
  assert.equal(withRef[0].dapp_anubis_staked, 9672.1541);
  assert.equal(without[0].dapp_anubis_staked, null);          // 못 읽으면 null → 화면에서 줄 숨김
  // 참조값 유무와 무관하게 보유·원금·redeem 합계는 동일해야 한다(오염 방지)
  assert.equal(withRef[0].holding_lgns, without[0].holding_lgns);
  assert.equal(withRef[0].principal_lgns, without[0].principal_lgns);
  assert.equal(withRef[0].redeemable_lgns, without[0].redeemable_lgns);
});
